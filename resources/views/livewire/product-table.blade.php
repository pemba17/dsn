            ...
            <div class="w-full sm:px-6">
                <div class="px-4 md:px-10 py-4 md:py-7 bg-gray-100 rounded-tl-lg rounded-tr-lg">
                    <div class="sm:flex items-center justify-between">
                        <p tabindex="0" class="focus:outline-none text-base sm:text-lg md:text-xl lg:text-2xl font-bold leading-normal text-gray-800">Products Table</p>
                        <div>
                            <a class="focus:ring-2 focus:ring-offset-2 focus:ring-indigo-600 inline-flex sm:ml-3 mt-4 sm:mt-0 items-start justify-start px-6 py-3 bg-indigo-700 hover:bg-indigo-600 focus:outline-none rounded" href="{{url('save-product')}}">
                    ...
                    <table class="w-full whitespace-nowrap">
                        <thead>
                            <tr tabindex="0" class="focus:outline-none h-16 w-full text-sm leading-none text-gray-800">
                                ...
                                <th class="font-normal text-left pl-12">Cost Price</th>
                                <th class="font-normal text-left pl-12">MRP Price</th>
                        ...
                        <tbody class="w-full">
                            @forelse($products as $key=>$row)
                                <tr tabindex="0" class="focus:outline-none h-20 text-sm leading-none text-gray-800 bg-white hover:bg-gray-100 border-b border-t border-gray-100">
                                    ...
                                    <td class="pl-8 cursor-pointer">
                                        <div class="flex items-center">
                                            @if($row->filename!=NULL || $row->filename!="")
                                                @php $image=explode(',',$row->filename);@endphp
                                                <div class="w-10 h-10">
                                                    <a href="{{url('view-product/'.$row->id)}}">
                                                        <img class="w-full h-full" src="{{asset($image[0])}}" alt="{{$row->name}}" />
                                                    </a>
                                                </div>
                                            @endif
                                            <div class="pl-4">
                                    ...
                                    <td class="pl-12">
                                        <p class="text-sm font-medium leading-none text-gray-800">Rs {{$row->cost_price}}</p>
                                    </td>

                                    <td class="pl-12">
                                        <p class="font-medium">From:Rs {{$row->mrp_from}}</p>
                                        <p class="font-medium mt-2">To:Rs {{$row->mrp_to}}</p>
                                    </td>
                                    <td class="pl-24">
                                    ...
                                    <td class="px-7 2xl:px-0 text-center" x-data="{open:false}">
                                        ...
                                        <div class="dropdown-content bg-white shadow w-24 absolute z-30 mr-6 mt-2" x-show="open">
                                            <div tabindex="0" class="focus:outline-none focus:text-indigo-600 text-xs w-full hover:bg-indigo-700 py-4 px-4 cursor-pointer hover:text-white" onclick="window.location='{{ url('save-product/'.$row->id) }}'">
                                                <span>Edit</span>
                                            </div>
